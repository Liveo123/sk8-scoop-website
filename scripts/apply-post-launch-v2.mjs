...
/* Post-launch reusable NUE continuation and local-photography components. */
.nue-next-section{padding-top:28px}
.nue-next-panel{background:linear-gradient(145deg,#fff,#f1f8f6);border:1px solid #d9e7e3;box-shadow:0 10px 24px rgba(19,45,49,.055)}
.nue-next-panel .button-row{margin-top:18px}
.nue-local-photo-section{padding-top:30px;padding-bottom:30px}
.nue-local-photo{display:grid;grid-template-columns:minmax(260px,.9fr) minmax(0,1.1fr);gap:28px;align-items:center;margin:0;padding:18px;background:#fff;border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 24px rgba(19,45,49,.06)}
.nue-local-photo img{display:block;width:100%;height:auto;max-height:360px;object-fit:contain;border-radius:13px;background:#f4f7f5}
.nue-local-photo figcaption{padding:4px 8px}
.nue-local-photo figcaption h2{margin:.2rem 0 .65rem}
.nue-local-photo figcaption p{margin:0 0 .75rem}
.nue-local-photo figcaption small{display:block;color:#526467;line-height:1.45}
body[data-page="whats-on"] .whats-on-local-filter-bar{display:flex;flex-wrap:wrap;gap:8px;margin:14px 0 24px}
body[data-page="whats-on"] .whats-on-local-filter-bar .button{min-height:40px;padding:9px 13px;font-size:.86rem}
body[data-page="whats-on"] .whats-on-local-filter-bar .button[aria-pressed="true"]{background:#073f48!important}
body[data-page="advertise"] .ad-product-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}
body[data-page="advertise"] .ad-product-card{height:100%}
body[data-page="advertise"] .ad-product-price{font:900 2.4rem/1 Arial,sans-serif;color:var(--teal);margin:.4rem 0 .75rem}
body[data-page="advertise"] .ad-product-price small{font:700 .85rem/1.35 Arial,sans-serif;color:#53666a}
body[data-page="advertise"] .ad-goal-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:18px 0}
body[data-page="advertise"] .ad-goal-grid a{display:block;text-align:center;padding:12px 10px;border:1px solid var(--line);border-radius:11px;text-decoration:none;font-weight:800;color:var(--teal);background:#fff}
body[data-page="advertise"] .ad-goal-grid a:hover{background:#f1f8f6}
@media(max-width:760px){.nue-local-photo{grid-template-columns:1fr}.nue-local-photo img{max-height:none}body[data-page="advertise"] .ad-product-grid{grid-template-columns:1fr}body[data-page="advertise"] .ad-goal-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
`;
